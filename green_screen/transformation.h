...
struct Transformation {
  ...
  // Construct from H
  Transformation(const cv::Matx44d &H)
      : R(H.get_minor<3, 3>(0, 0)), t(H(0, 3), H(1, 3), H(2, 3)) {}
  ...
  // Construct a transformation equivalent to this transformation followed by
  // the second transformation
  Transformation compose_with(
      const Transformation &second_transformation) const {
    // get this transform
